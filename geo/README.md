...
A PyVista-based visualizer is available to demonstrate SE(3) frame transformations using the `geo` library. It allows interactive manipulation of a lumbar phantom model using translation and rotation sliders.

To run the visualizer:

```bash
python scripts/visualizer/visualizer_main.py
```

This will open a window showing the phantom model. You can use the sliders to translate (Tx, Ty, Tz) and rotate (Roll, Pitch, Yaw) the model. The current transformation matrix is displayed in the upper right corner.
